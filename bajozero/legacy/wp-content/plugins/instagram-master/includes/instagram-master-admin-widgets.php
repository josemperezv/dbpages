<?php
function menu_single_instagram_admin_widgets(){
	if ( is_admin() )
	add_submenu_page( 'instagram-master', 'Widgets', 'Widgets', 'manage_options', 'instagram-master-admin-widgets', 'instagram_master_admin_widgets' );
}

function instagram_master_admin_widgets(){
$plugin_master_name = constant('INSTAGRAM_MASTER_NAME');
?>
<div class="wrap">
<h1>Widgets</h1>
<?php
if(!class_exists('instagram_master_admin_widgets_table')){
	require_once( dirname( __FILE__ ) . '/instagram-master-admin-widgets-table.php');
}
//Prepare Table of elements
$wp_list_table = new instagram_master_admin_widgets_table();
//Table of elements
...
if( is_multisite() ) {
add_action( 'admin_menu', 'menu_single_instagram_admin_widgets' );
}
else {
add_action( 'admin_menu', 'menu_single_instagram_admin_widgets' );
}
