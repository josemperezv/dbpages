<?php
function menu_single_instagram_admin_addons(){
	if ( is_admin() )
	add_submenu_page( 'instagram-master', 'Add-ons', 'Add-ons', 'manage_options', 'instagram-master-admin-addons', 'instagram_master_admin_addons' );
}

function instagram_master_admin_addons(){
$plugin_master_name = constant('INSTAGRAM_MASTER_NAME');
?>
<div class="wrap">
<h1><?php echo $plugin_master_name; ?> Add-ons</h1>
<?php
if(!class_exists('instagram_master_admin_addons_table')){
	require_once( dirname( __FILE__ ) . '/instagram-master-admin-addons-table.php');
}
//Prepare Table of elements
$wp_list_table = new instagram_master_admin_addons_table();
//Table of elements
...
if( is_multisite() ) {
add_action( 'admin_menu', 'menu_single_instagram_admin_addons' );
}
else {
add_action( 'admin_menu', 'menu_single_instagram_admin_addons' );
}
