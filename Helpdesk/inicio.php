          ...
          <br><br>
            <div class="span12">
              ...
              <div class="modal-body">
                  ...
                  <form name="ticket_record" method="POST" action="" onsubmit="SendData(); return false">
                      ...
                      <select name="InputClient">
                        <option value='0'></option>
                            <?php
                              ...
                              $results = $mysqli->query("SELECT Nombre,RIF from cliente");

                              while ($row = mysqli_fetch_assoc(@$results)){ 
                              ?>
                                <option value="<?php echo $row["RIF"];?>"><?php echo $row["Nombre"];?></option>
                              <?php
                              } 
                            ?>
                      ...
                      <select name="InputTech">
                        <option id="0" value='0'>Sin Asignar</option>
                            <?php
                              //establecer la conexion
                              $mysqli = new mysqli($host,$usuario,$contra,$db) or die ("No se puede conectar");
                              $results = $mysqli->query("SELECT Nombre_tecnico,Cedula from tecnico");

                              while ($row = mysqli_fetch_assoc(@$results)){ 
                              ?>
                                <option value="<?php echo $row["Cedula"];?>"><?php echo $row["Nombre_tecnico"];?></option>
                              <?php
                              } 
                            ?>
                        <!--<option id="0" value='0'>Sin Asignar</option>
                        <option id="1" value='18699438'>Miguelangel Rodriguez</option>                        
                        <option id="2" value='18735409'>Jose M Perez</option>-->
                      </select>
                      ...
                      <select name="InputPrio">
                        <option id="0" value='0'></option>
                            <?php
                              //establecer la conexion
                              $mysqli = new mysqli($host,$usuario,$contra,$db) or die ("No se puede conectar");
                              $results = $mysqli->query("SELECT ID_Prioridad,PTipo from prioridad");

                              while ($row = mysqli_fetch_assoc(@$results)){ 
                              ?>
                                <option value="<?php echo $row["ID_Prioridad"];?>"><?php echo $row["PTipo"];?></option>
                              <?php
                              } 
                            ?>
                      ...
                      <select name="InputCat">
                        <option id="0" value='0'></option>
                            <?php
                              //establecer la conexion
                              $mysqli = new mysqli($host,$usuario,$contra,$db) or die ("No se puede conectar");
                              $results = $mysqli->query("SELECT ID_Categoria,CTipo from categoria");

                              while ($row = mysqli_fetch_assoc(@$results)){ 
                              ?>
                                <option value="<?php echo $row["ID_Categoria"];?>"><?php echo $row["CTipo"];?></option>
                              <?php
                              } 
                            ?>
